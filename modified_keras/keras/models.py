...
class Graph(Model, containers.Graph):
    ...
    def train_on_batch(self, data, class_weight={}, sample_weight={}):
        ...
        outputs_with_losses = [o for o in self.output_order if o in self.loss_keys]
        sample_weight = [standardize_weights(data[name],
                                             sample_weight=sample_weight.get(name),
                                             class_weight=class_weight.get(name)) for name in outputs_with_losses]
        ins = [data[name] for name in self.input_order] + [standardize_y(data[name]) for name in outputs_with_losses] + sample_weight
        return self._train(*ins)

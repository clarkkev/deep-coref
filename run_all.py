...
def pretrain(model_props):
    if not already_trained('all_pairs', 'weights_140'):
        ...
        pairwise_learning.train(model_props=model_props, n_epochs=150)
    ...
    if not already_trained('top_pairs', 'weights_40'):
        ...
        pairwise_learning.train(model_props=model_props, n_epochs=50)


def make_predictions(model_props, datasets, save_scores=False):
    model_props.weights_file = 'best_weights'
...
def train_pairwise(model_props, mode='ranking'):
    ...
    pairwise_learning.train(model_props=model_props, n_epochs=100)
...
def acl2016():
    ...
    make_predictions(model_props, ["train", "dev", "test"], save_scores=False)
